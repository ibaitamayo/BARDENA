#' Estimate R0
#'
#' Estimates R0 through parametric method (EpiEstim) assuming mean serial interval=5.2 and standard dev of serial interval= 4.75
#'
#' @name tr
#'
...
tr= function(x,mean_si,std_si) {
  require(EpiEstim)
  require(dplyr)


  tr=estimate_R(dplyr::select(x,dates,I),  method="parametric_si", config = make_config(list(mean_si=mean_si , std_si=std_si)))
