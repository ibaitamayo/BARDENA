...
#' @name calculoR0
#'
#' @param tabla default file location of "Nuevos_positivos.csv"
#'
#' @return R0 results total and health area specific, filtered from a specific date.Includes options to change serial interval parameters.
#'
#' @examples
#'
#'
...
calculoR0<-function(tabla="//Centcs01srv03/TABLEAU/Coronavirus/R/Nuevos_positivos.csv",area=NULL,from_date=NULL,mean_si=5.2,std_si=4.75){
  require(readr)
  require(EpiEstim)
  require(dplyr)
  require(lubridate)
  ...
  covid_positives<- read_delim(tabla, ";", escape_double = FALSE, trim_ws = TRUE,  locale = locale(decimal_mark = ",",encoding = "ISO-8859-1"))
  ...
  if(!is.null(from_date)){
    covid_positives%>%filter(fdia>=ymd(from_date))->covid_positives
  }
  ...
  if(is.null(area)){
    resultado=tr(bbgac,mean_si,std_si)%>%mutate(area="todas_areas")
  }
  if(!is.null(area)){
    area_=tolower(area)
    ...
    resultado=tr(badtc%>%filter(area==area_)%>%dplyr::select(c("dates" ,"I" )),mean_si,std_si)%>%mutate(area=area)
  }
