...
comorb<-function(archivo,id="Pac_Unif_Cod",code="Diag_Cod", type="type",fecha="ResDiag_Inicio_Fecha",granularidad = TRUE,mini=NULL) {
  ...
  if(is.data.frame(archivo)){
    cat("Dataframe ==",is.data.frame(archivo))
    tabla<-archivo

  }else if(tools::file_ext(archivo)=="txt"){
    cat("txt file ==",tools::file_ext(archivo)=="txt")
  ...
  mpp[, c("id","fecha","code"):=tstrsplit(idfechacod, "," , fixed=TRUE)]
    ## eliminar registros con codigos no incluidos

  # mpp[, c("id","fecha","code"):=tstrsplit(x=idfechacod, "," , fixed=TRUE,names=TRUE)]
